...
export const commonSauces = [
  'Buffalo', 'BBQ', 'Honey BBQ', 'Teriyaki', 'Sweet & Sour',
  'Hot', 'Mild', 'Medium', 'Garlic Parmesan', 'Lemon Pepper',
  'Nashville Hot', 'Cajun', 'Asian Zing', 'Mango Habanero'
];
...
export function applyFilters(reviews: Review[], userLocation?: { latitude: number; longitude: number } | null): Review[] {
  ...
  return reviews.filter(review => {
    ...
    if (filters.selectedSauces.length > 0) {
      ...
      const hasMatchingSauce = filters.selectedSauces.some(sauce => 
        reviewSauces.some(reviewSauce => 
          reviewSauce.toLowerCase().includes(sauce.toLowerCase())
        )
      );
      if (!hasMatchingSauce) {
        return false;
...
export function applyQuickFilter(type: 'nearby' | 'top-rated' | 'recent', userLocation?: { latitude: number; longitude: number } | null) {
  switch (type) {
    case 'nearby':
      if (userLocation) {
