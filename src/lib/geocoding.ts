...
export async function geocode(
	query: string
): Promise<{ result: GeocodeResult | null; error: GeocodeError | null }> {
	try {
		...
		if (data && data.length > 0) {
			...
			return {
				...
				error: {
					type: 'no_results',
					message: `No location found for "${query}". Try being more specific or checking the spelling.`
				}
			};
		}
	} catch (error) {
		...
		return {
			...
			error: {
				type: 'network',
				message: 'Network error occurred while searching for location. Please try again.'
			}
		};
	}
}
