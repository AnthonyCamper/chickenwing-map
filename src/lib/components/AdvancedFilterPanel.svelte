<script lang="ts">
  ...
  import { 
    faTimes, faSliders, faStar, faMapMarkerAlt, 
    faClock, faThumbsUp, faUtensils, faCalendarAlt,
    faBookmark, faTrash, faSave
    ...
    >
      ...
      <div class="sticky top-0 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 px-6 py-4 z-10">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Advanced Filters</h2>
              {#if $activeFilterCount > 0}
                <p class="text-sm text-primary-600 dark:text-primary-400">{$activeFilterCount} active filter{$activeFilterCount !== 1 ? 's' : ''}</p>
              {/if}
            </div>
          </div>
          ...
          <div class="space-y-3">
            <label class="flex items-center gap-3">
              <input
                ...
                class="w-4 h-4 text-primary-600 bg-gray-100 border-gray-300 rounded focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
              />
              <span class="text-sm text-gray-700 dark:text-gray-300">Recent visits only (last 30 days)</span>
            </label>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="grid grid-cols-2 gap-2">
            {#each commonSauces as sauce}
              <label class="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors cursor-pointer">
                <input
                  type="checkbox"
                  value={sauce}
            ...
            <div class="mt-3 p-3 bg-primary-50 dark:bg-primary-900/20 rounded-lg">
              ...
              <div class="flex flex-wrap gap-1">
                {#each $filterState.selectedSauces as sauce}
                  <span class="inline-flex items-center gap-1 px-2 py-1 bg-primary-100 dark:bg-primary-800 text-primary-700 dark:text-primary-300 text-xs rounded-full">
                    {sauce}
                    <button
                      on:click={() => updateFilter('selectedSauces', $filterState.selectedSauces.filter(s => s !== sauce))}
      ...
      <div class="sticky bottom-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 px-6 py-4">
        <div class="flex gap-3">
          <Button variant="outline" size="md" fullWidth on:click={resetFilters}>
            Reset All
          </Button>
