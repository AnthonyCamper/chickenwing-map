...
{#if variant === 'switch'}
	...
	>
		<!-- Toggle circle -->
		<div
			class="relative {sizeClasses.toggle} {sizeClasses.translate} bg-white rounded-full shadow-lg transform transition-all duration-300 flex items-center justify-center"
		>
			<!-- Icon with smooth transition -->
			<div class="relative w-full h-full flex items-center justify-center">
				<!-- Sun icon -->
				<div
					class="absolute inset-0 flex items-center justify-center transition-all duration-300 {checked
		...
		<div
			class="absolute inset-0 rounded-full bg-gradient-to-r {checked
				? 'from-primary-400 to-primary-500'
				: 'from-gray-100 to-gray-200 dark:from-gray-600 dark:to-gray-700'} opacity-50 transition-all duration-300"
		></div>
	</button>
{:else}
	<!-- Button variant -->
	...
	>
		...
		<div
			class="absolute inset-0 rounded-full bg-gradient-to-br from-white/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"
		></div>
		...
		<div class="relative flex items-center justify-center">
			<!-- Sun icon -->
			<div
				class="absolute transition-all duration-500 {checked
					? 'opacity-100 rotate-0 scale-100'
