<script lang="ts">
	...
	import { searchResults } from '$lib/stores/searchStore';
	...
	export function zoomToLocation(latitude: number, longitude: number, locationName: string) {
		console.log(
			'zoomToLocation called with coords:',
			latitude,
			longitude,
			'and name:',
			locationName
	...
	function addMarkers() {
		...
		const locationReviews = reviews.reduce(
			(acc, review) => {
		...
		Object.values(locationReviews).forEach(({ location, reviews: locationReviews }) => {
			if (isValidCoordinate(location.latitude, location.longitude)) {
				const avgRating = (
					locationReviews.reduce((sum, r) => sum + parseFloat(r.rating), 0) / locationReviews.length
				).toFixed(1);
				// Create custom popup element
				const popupContent = document.createElement('div');
				popupContent.className =
					'bg-white dark:bg-gray-800 rounded-lg shadow-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors min-w-[220px] border border-gray-200 dark:border-gray-700';
	...
	function zoomToNearbyPlaces() {
		...
		if (map && L && userLocation) {
			const nearbyReviews = reviews.filter(
				(review) =>
					review.distance &&
					review.distance <= 10 &&
	...
	function highlightSearchResults(results: {
		...
		if (results.reviewMatches.length > 0) {
			...
			if (matchingLocationIds.size > 1) {
				const bounds = L.latLngBounds(
					results.reviewMatches.map((review) => [
...
<div class="relative w-full h-full map-container">
	<div
		...
		class:pr-0={!isSlideoutOpen}
		class:pr-80={isSlideoutOpen}
	></div>
	<button
