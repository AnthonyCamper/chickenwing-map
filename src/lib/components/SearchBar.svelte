<script lang="ts">
	...
	import { faSearch, faTimes, faMapMarkerAlt, faSpinner } from '@fortawesome/free-solid-svg-icons';
	...
	export let placeholder = 'Search restaurants, reviews, sauces, and more...';
	...
	function handleKeyPress(event: KeyboardEvent) {
		if (event.key === 'Enter') {
			...
			if ($searchQuery.trim().length >= 2) {
				performDebouncedSearch(reviews);
	...
	function search() {
		...
		performDebouncedSearch(reviews, true);
	...
	// Removed search mode toggle - now using unified search
...
<div class={`relative ${fullWidth ? 'w-full' : ''}`}>
	...
	<div class="relative group">
		<div
			class="relative flex items-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-full shadow-sm hover:shadow-md transition-all duration-200 focus-within:ring-2 focus-within:ring-primary-500/20 focus-within:border-primary-500"
		>
			<!-- Search icon -->
	...
	{#if $searchError}
		<div
			class="absolute z-[1002] mt-3 w-full bg-white dark:bg-gray-900 border border-red-200 dark:border-red-800 rounded-lg shadow-lg p-4"
		>
			<div class="flex items-start gap-3">
				<div
					class="flex-shrink-0 w-8 h-8 bg-red-50 dark:bg-red-900/20 rounded-full flex items-center justify-center"
				>
	...
	{#if $shouldShowNoResults && !$searchError}
		<div
			class="absolute z-[1002] mt-3 w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-4"
		>
			<div class="flex items-center gap-3">
				<div
					class="flex-shrink-0 w-8 h-8 bg-gray-50 dark:bg-gray-800 rounded-full flex items-center justify-center"
				>
					<Icon icon={faSearch} class="h-4 w-4 text-gray-400 dark:text-gray-500" />
				</div>
				<div class="flex-1">
		...
		>
			<div class="flex items-center gap-3">
				<div class="flex-shrink-0">
					<Icon
						icon={faSpinner}
						class="h-4 w-4 text-primary-600 dark:text-primary-400 animate-spin"
					/>
				</div>
	...
	{#if $shouldShowResults && showDropdown}
		<div
			class="absolute z-[1002] mt-3 w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg overflow-hidden max-h-[420px]"
		>
			<!-- Results header -->
			<div
			...
			>
				<p class="text-xs font-medium text-gray-700 dark:text-gray-300">
					Found {$searchResults.reviewMatches.length} result{$searchResults.reviewMatches.length !==
					1
						? 's'
						: ''} for "{$searchQuery}"
				</p>
			...
			{#if $searchResults.reviewMatches.length > 0}
				<div class="overflow-y-auto max-h-[320px]">
					{#each $searchResults.reviewMatches.slice(0, 6) as review (review.id)}
						<div
							class="group px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors border-b border-gray-100 dark:border-gray-800 last:border-b-0"
						>
							<div class="flex items-start justify-between gap-3">
								<!-- Restaurant info -->
								<div class="flex items-start gap-3 flex-1 min-w-0">
									<div
										class="flex-shrink-0 w-8 h-8 bg-primary-50 dark:bg-primary-900/20 rounded-full flex items-center justify-center"
									>
										<Icon
											icon={faMapMarkerAlt}
				...
				{#if $searchResults.reviewMatches.length > 6}
					...
					>
						<p class="text-xs text-center text-gray-500 dark:text-gray-400">
							{$searchResults.reviewMatches.length - 6} more result{$searchResults.reviewMatches
								.length -
								6 !==
							1
								? 's'
	...
	{#if shouldShowRecent}
		<div
			class="absolute z-[1002] mt-3 w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg overflow-hidden"
		>
			...
			<div class="py-1">
				{#each $recentSearches as query (query)}
					...
					>
						<div class="flex items-center gap-3">
							<div
								class="flex-shrink-0 w-6 h-6 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center"
							>
