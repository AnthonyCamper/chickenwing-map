<script lang="ts">
  import { faStar, faMapMarkerAlt, faCalendarAlt, faThumbsUp, faThumbsDown } from '@fortawesome/free-solid-svg-icons';
...
<div
  class="group relative bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-6 transition-all duration-300 hover:shadow-lg hover:shadow-gray-200/50 dark:hover:shadow-gray-900/50 hover:-translate-y-0.5 cursor-pointer
         {isSelected ? 'ring-2 ring-primary-500 ring-offset-2 ring-offset-gray-50 dark:ring-offset-gray-900 shadow-lg' : 'hover:border-gray-300 dark:hover:border-gray-600'}"
  on:click
  role="button"
  tabindex="0"
  on:keydown
>
  ...
  <div class="flex items-start justify-between mb-3">
    <div class="flex-1 min-w-0">
      <h3 class="font-semibold text-lg text-gray-900 dark:text-white truncate group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
    ...
    <div class="flex-shrink-0 ml-4">
      <div class="inline-flex items-center gap-2 bg-gradient-to-r from-primary-50 to-primary-100 dark:from-primary-900/30 dark:to-primary-800/30 text-primary-700 dark:text-primary-300 px-3 py-2 rounded-full border border-primary-200 dark:border-primary-700">
  ...
  <div class="flex items-center gap-1 mb-3">
    {#each stars as star}
      <span class="text-lg {star === 'full' ? 'text-yellow-400' : star === 'half' ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'}">
        {star === 'full' ? '★' : star === 'half' ? '☆' : '☆'}
      </span>
    {/each}
  ...
  <div class="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-gray-800">
    ...
    <!-- View indicator -->
    <div class="flex items-center text-xs text-gray-400 dark:text-gray-500 group-hover:text-primary-500 dark:group-hover:text-primary-400 transition-colors">
  ...
  <div class="absolute inset-0 bg-gradient-to-r from-primary-500/5 to-primary-600/5 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
</div>
