<script lang="ts">
		...
		try {
			...
			if (err) {
				...
				if (data && data.length === 0) {
					error =
						'No users found matching that email. You can still add users directly by UUID using the + button.';
	...
	async function addUserById() {
		...
		try {
			...
			const { error: err } = await supabase.from('authorized_users').insert({
				user_id: userId,
	...
	async function removeUser(userId: string, displayName: string | null, email: string | null) {
		...
		try {
			const { error: err } = await supabase.from('authorized_users').delete().eq('user_id', userId);
	...
	function getPermissionBadgeClass(user: AuthorizedUser): string {
		...
		if (user.can_delete_reviews)
			return 'bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300';
		if (user.can_edit_reviews)
		...
		<div class="flex justify-between items-center mb-6">
			<div>
				...
				<p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
					Manage user permissions for adding, editing, and deleting reviews
				</p>
			</div>
			<button
			...
			{#if searchResults.length > 0}
				<div class="mt-3 space-y-2">
					...
					{#each searchResults as user}
						<div
							class="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-800 rounded border"
						>
							...
							<button class="btn-primary btn-sm" on:click={() => addUser(user.user_id, user.email)}>
			...
			{:else}
				<div class="space-y-3 max-h-96 overflow-y-auto">
					{#each authorizedUsers as user}
						<div
							class="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50"
						>
							<div class="flex items-center space-x-4">
								...
								<div class="flex-1">
									<div class="flex items-center space-x-2">
										...
										<span
											class="px-2 py-1 text-xs font-medium rounded-full {getPermissionBadgeClass(
												user
											)}"
										>
		...
		<div class="modal-content max-w-md">
			...
			<div class="mb-4">
				<p class="text-sm text-gray-600 dark:text-gray-400">
					{selectedUser.display_name || 'User'} • {selectedUser.email ||
			...
			<div class="space-y-4 mb-6">
				<label class="flex items-center space-x-3">
					<input type="checkbox" class="form-checkbox" bind:checked={editForm.is_admin} />
					<div>
				...
				<label class="flex items-center space-x-3">
					<input type="checkbox" class="form-checkbox" bind:checked={editForm.can_add_reviews} />
					<div>
				...
				<label class="flex items-center space-x-3">
					<input type="checkbox" class="form-checkbox" bind:checked={editForm.can_edit_reviews} />
					<div>
				...
				<label class="flex items-center space-x-3">
					<input type="checkbox" class="form-checkbox" bind:checked={editForm.can_delete_reviews} />
					<div>
						<span class="text-sm font-medium text-gray-900 dark:text-white">Delete Reviews</span>
						<p class="text-xs text-gray-500 dark:text-gray-400">
							Can remove reviews (destructive action)
						</p>
					</div>
				</label>
			...
			<div class="flex justify-end space-x-3">
				...
				<button class="btn-primary" on:click={savePermissions}> Save Changes </button>
			</div>
		</div>
	</div>
{/if}
