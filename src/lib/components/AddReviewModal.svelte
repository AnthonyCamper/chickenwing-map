			...
			const result = await Promise.race([
				(async () => {
					...
					if (authError || !authorizedUser) {
						throw new Error(
							'You are not authorized to add reviews. Please contact an administrator to request access.'
						);
					}

					if (!authorizedUser.can_add_reviews && !authorizedUser.is_admin) {
						throw new Error(
							'You do not have permission to add reviews. Please contact an administrator.'
						);
					}
	...
	<div class="modal-container" transition:fly={{ y: 20, duration: 300 }}>
		...
		<div class="modal-content">
			...
			<form on:submit|preventDefault={handleSubmit} class="space-y-8">
				...
				<ReviewSauceDetails bind:sauceAvailability bind:selectedSauces />
