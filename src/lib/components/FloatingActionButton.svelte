<script lang="ts">
	...
	$: variantClasses = {
		...
		secondary:
			'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 shadow-lg hover:shadow-xl hover:bg-gray-50 dark:hover:bg-gray-700'
	}[variant];
</script>

<div class="fixed {positionClasses} z-50">
	<div
		class="flex items-center gap-3 {showLabel && position.includes('right')
			? 'flex-row-reverse'
			: ''}"
	>
		<!-- Main FAB button -->
		<button
			class="group relative {sizeClasses.button} {variantClasses} rounded-full transition-all duration-300 transform hover:scale-110 active:scale-95 focus:outline-none focus:ring-4 focus:ring-primary-500/20 {pulse
				? 'animate-pulse'
				: ''}"
		...
		>
			...
			<div
				class="absolute inset-0 rounded-full bg-gradient-to-r from-primary-400 to-primary-500 opacity-0 group-hover:opacity-20 transition-opacity duration-300 blur-xl"
			></div>
			...
			<div class="relative flex items-center justify-center w-full h-full">
				<Icon
					{icon}
					class="{sizeClasses.icon} transition-transform duration-300 group-hover:rotate-90"
				/>
			</div>

			<!-- Ripple effect on click -->
			<div
				class="absolute inset-0 rounded-full bg-white/20 scale-0 opacity-0 group-active:scale-100 group-active:opacity-100 transition-all duration-150"
			></div>
		</button>
