...
<div class="flex items-center space-x-4">
	...
	{#if user}
		...
		<div class="relative user-menu">
			...
			>
				<div
					class="w-8 h-8 rounded-full bg-primary-100 dark:bg-primary-900 flex items-center justify-center text-primary-700 dark:text-primary-300"
				>
					{#if displayName}
	...
	{:else}
		<!-- Sign In Button -->
		<button class="btn-primary text-sm px-3 py-1.5" on:click={() => (showSignInModal = true)}>
			Sign In
		</button>
	{/if}
</div>
	...
	<div class="modal">
		...
		<div class="modal-content max-w-md">
			...
			<div class="mb-6">
				...
				<input type="text" id="displayName" class="form-input" bind:value={tempDisplayName} />
				{#if settingsError}
...
{#if showPermissionsManager}
	<PermissionsManager onClose={() => (showPermissionsManager = false)} currentUserId={user?.id} />
{/if}
